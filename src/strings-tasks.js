...
function getStringLength( value ) {
  if (value === null || value === undefined) { return 0; } return value.length;
}
...
function isPalindrome( str ) {
  ...
  const len = sanitizedStr.length;
  let res = '';
  for (let i = sanitizedStr.length-1; i >= 0; i -= 1) {
    if (sanitizedStr[i] === sanitizedStr[len - 1 - i]) {
      res += sanitizedStr[len - 1 - i];
    }
  }
 return sanitizedStr === res ? true : false;
}
